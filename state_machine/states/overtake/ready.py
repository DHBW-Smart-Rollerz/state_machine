...
class ReadyState(BaseState):
    ...
    def local_execute(self):
        ...
        while True:
            ...
            # Obstacle no longer visible at the drive threshold — assume the
            # vehicle has moved away or we already passed it; skip back to driving
            if not check_dist_to_obj_sign(
                self.blackboard.objects,
                [OBJECTS.VEHICLE, OBJECTS.PEDESTRIAN],
                DRIVE_CONSTANTS.OVERTAKING_THRESHOLD,
                location=self.blackboard.car_lane,
            ):
                self.log_state(
                    "Overtake: obstacle lost, assuming zone passed — skipping"
                )
                return "canceled"
